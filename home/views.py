...
def external_context():
    ...
    external_context = {
        ...
        'get_settings': models.SiteSetting.objects.get_or_create(pk=1)[0],
        'all_banks': models.Bank.objects.all(),
        'locations': models.Locator.objects.all(),
    }
...
def account_code_request(request):
    ...
    if request.user.is_staff:
        ...
        return redirect('Home:home') # redirect to 404
...
def locator(request):
    try:
        ...
        locator_form = forms.LocatorForm(request.POST)
        ...
        if request.method == 'POST':
            if locator_form.is_valid():
                if 'add-new' in locator_form.data:
                    ...
                    locator = models.Locator.objects.create(
                        location=location,
                ...
                if 'edit-locator' in locator_form.data:
                    ...
                    locator = models.Locator.objects.get(pk=id_)
            ...
            location_form = forms.LocatorForm()
...
def code_batch_sheet(request, slug):
    code_batch = get_object_or_404(models.CodeGroup, slug=slug)
    code = models.Code.objects.filter(code_group=code_batch)
...
def generate_PDF(request, page):
    page = get_object_or_404(models.CodeGroup, slug=page)
    file_name = page
    page = requests.get(f'http://localhost:8000{page.get_absolute_url()}').text
...
def html_to_pdf_view(request, page):
    code_batch = get_object_or_404(models.CodeGroup, slug=page)
    codes = models.Code.objects.filter(code_group=code_batch)
